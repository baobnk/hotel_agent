...
import { ReasoningFlowModal, ViewReasoningButton, ReasoningFlowData } from "./reasoning-flow-modal";
...
export interface Message {
  ...
  hotels?: Hotel[];
  isStreaming?: boolean;
  reasoningData?: ReasoningFlowData;
}
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const hasReasoningData = message.reasoningData && !message.isStreaming;
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* View Reasoning Flow button - only show when we have results and reasoning data */}
          {hasReasoningData && (
            <div className="flex justify-start px-1">
              <ViewReasoningButton onClick={() => setShowReasoningModal(true)} />
            </div>
          )}
        </div>
      ...
      {/* Reasoning Flow Modal */}
      {hasReasoningData && message.reasoningData && (
        <ReasoningFlowModal
          isOpen={showReasoningModal}
          onClose={() => setShowReasoningModal(false)}
          data={message.reasoningData}
        />
      )}
    </>
  );
